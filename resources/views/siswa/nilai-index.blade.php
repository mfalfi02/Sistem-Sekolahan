    ...
    <section class="rounded-3xl border border-white/10 bg-white/5 p-8 shadow-2xl backdrop-blur-xl">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
                ...
                <p class="mt-2 text-sm text-teal-200/80">Nilai akhir sudah memasukkan absensi per mapel sebesar 10%.</p>
            </div>
        </div>

        @if (! empty($periodFilters) && collect($periodFilters)->isNotEmpty())
            <div class="mt-6 flex flex-wrap gap-3">
                <a
                    href="{{ route('siswa.nilai.index') }}"
                    class="rounded-xl border px-4 py-2 text-sm font-semibold transition {{ empty($selectedPeriodKey) ? 'border-teal-300/20 bg-teal-300/15 text-teal-100' : 'border-white/10 bg-white/5 text-slate-200 hover:bg-white/10' }}"
                >
                    Semua Semester
                </a>
                @foreach ($periodFilters as $period)
                    <a
                        href="{{ route('siswa.nilai.index', ['periode' => $period['key']]) }}"
                        class="rounded-xl border px-4 py-2 text-sm font-semibold transition {{ ($selectedPeriodKey ?? '') === $period['key'] ? 'border-teal-300/20 bg-teal-300/15 text-teal-100' : 'border-white/10 bg-white/5 text-slate-200 hover:bg-white/10' }}"
                    >
                        <span class="block">{{ $period['label'] }}</span>
                        <span class="block text-xs font-normal text-slate-400">{{ $period['caption'] }}</span>
                    </a>
                @endforeach
            </div>
        @endif
            ...
            <section data-table-filter class="overflow-hidden rounded-3xl border border-white/10 bg-slate-900/60 shadow-2xl">
                ...
                <div class="overflow-x-auto">
                    ...
                    <tbody class="divide-y divide-white/10">
                        @forelse ($period['records'] as $item)
                            @php
                                $attendanceContribution = round(((float) ($item->nilai_absensi ?? 0)) * 0.10, 2);
                            @endphp
                            <tr data-table-filter-row class="hover:bg-white/5">
                                ...
                                <td class="px-6 py-4 text-sm text-slate-300">
                                    {{ number_format((float) ($item->persentase_absensi ?? 0), 2) }}%
                                    <span class="block text-xs text-slate-500">
                                        {{ (int) ($item->absensi_hadir ?? 0) }}/{{ (int) ($item->absensi_total ?? 0) }} hadir
        ...
        <section data-table-filter class="overflow-hidden rounded-3xl border border-white/10 bg-slate-900/60 shadow-2xl">
            <div class="border-b border-white/10 px-6 py-5">
                <div class="grid gap-5 xl:grid-cols-[1fr_auto] xl:items-center">
                    <div>
                        <p class="text-xs uppercase tracking-[0.24em] text-teal-200/70">Semester</p>
                        <h3 class="mt-2 text-2xl font-semibold">-</h3>
                    ...
                    <div class="grid gap-3 sm:grid-cols-4 xl:w-[620px]">
                        <div class="rounded-2xl border border-teal-300/20 bg-teal-300/10 p-4 sm:col-span-2">
                            <div class="flex items-end justify-between gap-3">
                                <div>
                                    <p class="text-xs font-semibold uppercase tracking-[0.2em] text-teal-200/70">Rata-rata</p>
                                    <p class="mt-2 text-4xl font-bold text-teal-200">0.00</p>
                                </div>
                                <p class="text-sm font-semibold text-slate-300">/100</p>
                            </div>
                            <div class="mt-3 h-2 overflow-hidden rounded-full bg-slate-950/50">
                                <div class="h-full rounded-full bg-teal-300" style="width: 0%"></div>
                            </div>
                        </div>
                        <div class="rounded-2xl border border-white/10 bg-white/5 p-4">
                            <p class="text-xs text-slate-400">Tuntas</p>
                            <p class="mt-2 text-3xl font-bold text-emerald-200">0</p>
                        </div>
                        <div class="rounded-2xl border border-white/10 bg-white/5 p-4">
                            <p class="text-xs text-slate-400">Remedial</p>
                            <p class="mt-2 text-3xl font-bold text-amber-200">0</p>
                        </div>
                    </div>
                </div>
                <div class="mt-4 flex flex-wrap items-center gap-2">
                    <span class="rounded-full bg-white/10 px-3 py-1 text-xs font-semibold text-slate-300">0 mapel</span>
                    <span class="rounded-full bg-emerald-300/10 px-3 py-1 text-xs font-semibold text-emerald-200">0 tuntas</span>
                    <span class="rounded-full bg-amber-300/10 px-3 py-1 text-xs font-semibold text-amber-200">0 perlu remedial</span>
                </div>
            </div>
            <div class="border-b border-white/10 px-6 py-4">
                <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
