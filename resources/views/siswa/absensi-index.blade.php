            ...
            <section data-table-filter class="overflow-hidden rounded-3xl border border-white/10 bg-slate-900/60 shadow-2xl">
                <div class="border-b border-white/10 px-6 py-5">
                    <div class="grid gap-5 xl:grid-cols-[1fr_auto] xl:items-center">
                        <div>
                            ...
                            <p class="mt-2 text-xs text-slate-500">{{ $period['total'] }} catatan absensi</p>
                        </div>
                ...
                <div class="border-b border-white/10 px-6 py-4">
                    <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                        ...
                        <input type="search" data-table-filter-input placeholder="Cari absensi..." class="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 tekst-sm text-white placeholder-slate-500 outline-none focus:border-teal-300/50 lg:max-w-sm">
                    </div>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-white/10 text-left">
                        ...
                        <tbody class="divide-y divide-white/10">
                            ...
                            <tr data-table-filter-row class="hover:bg-white/5">
                                <td class="px-6 py-4 text-sm text-slate-300">{{ \App\Support\IndonesianDateTime::date($item->tanggal_absen) }}</td>
                                <td class="px-6 py-4 text-sm font-semibold text-slate-200">{{ ucfirst($item->status_kehadiran) }}</td>
                                <td class="px-6 py-4 text-sm text-slate-300">
                                    <div>{{ $item->guru?->nama_guru ?? '-' }}</div>
...
<<<<<<< ours
                            ...
                            <div class="mt-3 h-2 overflow-hidden rounded-full bg-slate-950/50">
                                <div class="h-full rounded-full bg-teal-300" style="width: {{ min(100, max(0, $attendancePercent)) }}%"></div>
=======
                                <p class="text-4xl font-bold text-teal-200">0%</p>
                                <p class="text-sm font-semibold text-slate-300">0/0</p>
                            </div>
                            <div class="mt-3 h-2 overflow-hidden rounded-full bg-slate-950/50">
...
<<<<<<< ours
                                <p class="text-2xl font-bold {{ $status === 'hadir' ? 'text-emerald-200' : ($status === 'terlambat' ? 'text-amber-200' : 'text-slate-100') }}">{{ $period['summary']->get($status, 0) }}</p>
=======
                                <p class="text-2xl font-bold {{ $status === 'hadir' ? 'text-emerald-200' : ($status === 'terlambat' ? 'text-amber-200' : 'text-slate-100') }}">0</p>
>>>>>>> theirs
                                <p class="mt-1 text-xs text-slate-400">{{ $label }}</p>
                            </div>
                        @endforeach
