<section class="rounded-3xl border border-white/10 bg-white/5 p-8 shadow-2xl backdrop-blur-xl">
    ...
    <p class="mt-2 text-sm text-slate-400">Welcome {{ $guruName ?? 'Guru' }}</p>
</section>
        ...
        <div class="space-y-3">
            @forelse ($recentNilai ?? [] as $nilai)
                <div class="flex items-center gap-3 p-3 rounded-xl bg-white/5">
                    ...
                    <div>
                        <p class="font-medium text-slate-200">{{ $nilai->siswa?->nama_siswa }} <span class="text-slate-400">({{ $nilai->siswa?->kelas?->nama_kelas ?? '-' }})</span> - {{ $nilai->nilai }}</p>
                        <p class="text-sm text-slate-400">{{ $nilai->mataPelajaran?->nama_mapel ?? '-' }}</p>
